      ...
      <div className="container mx-auto max-w-[1280px] p-6 md:p-10 flex flex-col gap-8 pb-24">
        ...
        {courses.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-slate-400">
            ...
            <p className="text-lg font-medium">目前沒有可用的課程</p>
            <p className="text-sm">請確認您已綁定老師，或聯繫您的老師開設課程。</p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
          {courses.map((course, index) => {
            ...
            return (
              <div key={course.id} className="bg-surface-light dark:bg-surface-dark rounded-3xl border border-slate-200 dark:border-slate-700 shadow-soft overflow-hidden group flex flex-col h-full hover:border-primary/50 dark:hover:border-primary/50 trans-all">
                <div className="h-56 relative overflow-hidden">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                    <h3 className="text-white text-2xl font-bold leading-tight shadow-black drop-shadow-md mb-2">{course.title}</h3>
                    <div className="flex items-center gap-4 text-white/90 text-sm font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-75">
                      <span className="flex items-center gap-1"><span className="material-symbols-outlined text-[16px]">schedule</span> {course.durationMinutes ? `${course.durationMinutes} 分鐘/堂` : '彈性時間'}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-6 flex flex-col gap-6 flex-1">
                  <div className="space-y-4">
                    ...
                    {/* Hour Selection */}
                    <div className="pt-2">
                      <label className="text-xs font-bold text-slate-500 dark:text-slate-400 mb-2 block">選擇預約時數</label>
                      <div className="flex items-center gap-3 bg-slate-100 dark:bg-slate-800 p-1.5 rounded-xl border border-slate-200 dark:border-slate-700">
                        ...
                        <div className="flex-1 text-center">
                          <span className="text-lg font-black text-slate-800 dark:text-white">{currentHours}</span>
                  ...
                  <div className="mt-auto pt-6 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                    <div className="flex flex-col">
                      <span className="text-xs text-slate-400 font-bold uppercase">預估費用</span>
                      <div className="flex items-baseline gap-1">
                        <span className="text-2xl font-black text-primary-dark dark:text-primary">
                          {course.price ? `NT$ ${totalPrice.toLocaleString()}` : '洽詢'}
                        </span>
                      </div>
                    </div>
                    <button 
        ...
        <div className="mt-8 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-3xl p-8 md:p-12 text-white relative overflow-hidden shadow-xl">
           ...
           <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
             <div className="max-w-2xl">
               <h2 className="text-3xl font-display font-bold mb-4">需要客製化課程？</h2>
               <p className="text-indigo-100 text-lg">如果您有特殊的學習需求，歡迎直接與老師聯繫討論。</p>
             </div>
